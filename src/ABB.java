...
public class ABB<K, V> implements IMapeamento<K, V> {
    ...
    /**
     * Constrói uma ABB a partir de outra estrutura de produtos, utilizando um extrator para definir
     * as novas chaves.
     * @param origem árvore de origem com os itens que serão copiados.
     * @param extratorDeChave função que extrai, de cada item, a chave da nova árvore.
     */
    public <KOrigem> ABB(ABB<KOrigem, V> origem, Function<V, K> extratorDeChave) {
    	this();
    	if (origem != null && extratorDeChave != null) {
    		inserirItensMapeados(origem.raiz, extratorDeChave);
    	}
    }

    private <KOrigem> void inserirItensMapeados(No<KOrigem, V> raizArvore, Function<V, K> extratorDeChave) {
    	if (raizArvore == null)
    		return;

    	inserirItensMapeados(raizArvore.getEsquerda(), extratorDeChave);
    	K novaChave = extratorDeChave.apply(raizArvore.getItem());
    	inserir(novaChave, raizArvore.getItem());
    	inserirItensMapeados(raizArvore.getDireita(), extratorDeChave);
    }
    ...
    public Lista<V> recortar(K chaveDeOnde, K chaveAteOnde) {
        Lista<V> itensRecortados = new Lista<>();

        if (chaveDeOnde == null || chaveAteOnde == null)
            return itensRecortados;

        if (comparador.compare(chaveDeOnde, chaveAteOnde) > 0)
            return itensRecortados;

        recortar(raiz, chaveDeOnde, chaveAteOnde, itensRecortados);
        return itensRecortados;
    }

    private void recortar(No<K, V> raizArvore, K chaveDeOnde, K chaveAteOnde, Lista<V> itensRecortados) {

        if (raizArvore == null)
            return;

        if (comparador.compare(chaveDeOnde, raizArvore.getChave()) < 0)
            recortar(raizArvore.getEsquerda(), chaveDeOnde, chaveAteOnde, itensRecortados);

        if (comparador.compare(chaveDeOnde, raizArvore.getChave()) <= 0
                && comparador.compare(raizArvore.getChave(), chaveAteOnde) <= 0)
            itensRecortados.inserir(raizArvore.getItem());

        if (comparador.compare(raizArvore.getChave(), chaveAteOnde) < 0)
            recortar(raizArvore.getDireita(), chaveDeOnde, chaveAteOnde, itensRecortados);
    }
